...
const getUserArticles =
  ...
  <T extends UserType>({ username }: T): Promise<QueryResult<any>> => {
    const sql: QueryConfig<any[]> = {
      text: `SELECT a.id, a.title, a.slug, a.temp_slug,a.status, a.temp_preview_link FROM articles AS a INNER JOIN users AS u ON u.id = a.user_id WHERE u.username = $1;`,
      values: [username],
    };
