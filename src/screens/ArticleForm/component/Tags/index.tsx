...
const Tags: FC<{
  ...
  function handleChange(evt: React.ChangeEvent<HTMLInputElement> | React.BaseSyntheticEvent<any>) {
    var { value } = evt.target;

    if (evt.target.selectionStart !== null) {
      setInputValue(value);
      const isTypingAfterComma = evt.target.value[evt.target.selectionStart - 2] === ',';
    ...
    if (evt.target.selectionStart != null) {
      const query = getCurrentTagAtSelectionIndex(evt.target.value, evt.target.selectionStart - 1);
  ...
  const getRangeBetweenCommas = (value: string, index: number | null) => {
    ...
    if (index !== null) {
      if (index !== undefined && index !== null) {
        toPreviousComma = value.slice(0, index).split('').reverse().indexOf(',');
      }
  ...
  const insertTag = (tag: string) => {
    ...
    const newInput =
      input.value.slice(0, range[0]) + tagValue + input.value.slice(range[1], input.value.length);
  ...
  const fetchTopTagSuggestions = useCallback(() => {
    if (tagsList.length > 0) {
      return setSearchResults(
        tagsList.filter(t => !selectedTags.includes(t.name) && t.name.includes(searchQuery)),
      );
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const search = (query: string) => {
    ...
    if (handleSelecteTags().length > 0) {
      arr = tagsList.filter(t => t.name === query || !selectedTags.includes(t.name));
  ...
  const searchResultElm = searchResults.map(tag => {
    return (
      <div key={`option-${tag.name}`} tabIndex={-1} className={`${classes.searchResultRow}`}>
        <div
          role="button"
